""" 
Run the sequential Serinv codes on GPU using the inlamat matrices.
"""
...
if __name__ == "__main__":
    ...
    tic = time.perf_counter()
    A_diagonal_blocks = cp.zeros_like(A_diagonal_blocks_init)
    A_lower_diagonal_blocks = cp.zeros_like(A_lower_diagonal_blocks_init)
    A_arrow_bottom_blocks = cp.zeros_like(A_arrow_bottom_blocks_init)
    A_arrow_tip_block = cp.zeros_like(A_arrow_tip_block_init)
    toc = time.perf_counter()
    ...
    for i in range(n_warmups + n_iterations):
        ...
        tic = time.perf_counter()
        A_diagonal_blocks[:, :, :] = cp.asarray(A_diagonal_blocks_init[:, :, :])
        A_lower_diagonal_blocks[:, :, :] = cp.asarray(A_lower_diagonal_blocks_init[:, :, :])
        A_arrow_bottom_blocks[:, :, :] = cp.asarray(A_arrow_bottom_blocks_init[:, :, :])
        A_arrow_tip_block[:, :] = cp.asarray(A_arrow_tip_block_init[:, :])
        toc = time.perf_counter()
