#!/bin/bash
#SBATCH --job-name=cpu_ppobtax_2
#SBATCH --nodes=8             #Number of Nodes desired e.g 1 node
...
srun --cpu-bind=socket python run_cpu_ppobtax.py --diagonal_blocksize $DIAGONAL_BLOCKSIZE --arrowhead_blocksize $ARROWHEAD_BLOCKSIZE --n_diag_blocks $N_DIAG_BLOCKS --file_path $FILE_PATH --n_iterations $N_ITERATIONS --density $DENSITY --strategy $STRATEGY
