...
def extract_partition(
    ...
    A_arrow_bottom = A_global[
        -arrow_blocksize:-1, start_blockrow * blocksize : stop_blockrow * blocksize
    ]
    A_arrow_right = A_global[
        start_blockrow * blocksize : stop_blockrow * blocksize, -arrow_blocksize:-1
    ]
...
def update_sinv_reduced_system(
    ...
    if process == 0:
        ...
        Bridges_upper[process, :, :] = reduced_system[
            :blocksize, blocksize : 2 * blocksize
        ]

        S_arrow_bottom[arrow_blocksize, -blocksize:] = reduced_system[
            -arrow_blocksize:, :blocksize
        ]
        S_arrow_right[arrow_blocksize, -blocksize:] = reduced_system[
            :blocksize, -arrow_blocksize:
        ]

    else:
        start_index = blocksize + (process - 1) * 2 * blocksize
        Bridges_lower[process] = reduced_system[
            start_index : start_index + blocksize, start_index - blocksize : start_index
        ]
