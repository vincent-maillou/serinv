...
def chol_slv_tridiag(
    ...
    X[0:blocksize] = la.solve_triangular(L[0:blocksize, 0:blocksize], B[0:blocksize], lower=True)
    for i in range(1, n_blocks, 1):
        ...
        X[i*blocksize:(i+1)*blocksize] = la.solve_triangular(
            ...
            X[i*blocksize:(i+1)*blocksize]-L[i*blocksize:(i+1)*blocksize, (i-1)*blocksize:i*blocksize] @ X[(i-1)*blocksize:(i)*blocksize], 
            lower=True
        )
    ...
    for i in range(n_blocks-2, -1, -1):
        ...
        X[i*blocksize:(i+1)*blocksize] = la.solve_triangular(
            L[i*blocksize:(i+1)*blocksize, i*blocksize:(i+1)*blocksize], 
            X[i*blocksize:(i+1)*blocksize]-L[(i+1)*blocksize:(i+2)*blocksize, i*blocksize:(i+1)*blocksize].T @ X[(i+1)*blocksize:(i+2)*blocksize], 
            lower=True, 
            trans='T'
        )
...
def chol_slv_ndiags(
    ...
    for i in range(1, n_blocks, 1):
        ...
        for k in range(max(0,i-n_offdiags_blk), i, 1):
            B_temp = B_temp - L[i*blocksize:(i+1)*blocksize, k*blocksize:(k+1)*blocksize] @ X[k*blocksize:(k+1)*blocksize]
    ...
    for i in range(n_blocks-2, -1, -1):
        ...
        for k in range(i+1, min(i+1+n_offdiags_blk, n_blocks)):
            B_temp = B_temp - L[k*blocksize:(k+1)*blocksize, i*blocksize:(i+1)*blocksize].T @ X[k*blocksize:(k+1)*blocksize]
...
def chol_slv_ndiags_arrowhead(
    ...
    X[0:diag_blocksize] = la.solve_triangular(L[0:diag_blocksize, 0:diag_blocksize], X[0:diag_blocksize], lower=True)
    for i in range(1, n_diag_blocks, 1):
        # Y_{i} = L_{i,i}^{-1} (B_{i} - sum_{k=max(0,i-offdiags)}^{i-1} L_{i,k} Y_{k}) 
        B_temp[:] = X[i*diag_blocksize:(i+1)*diag_blocksize]
        for k in range(max(0,i-n_offdiags_blk), i, 1):
            B_temp[:] = B_temp - L[i*diag_blocksize:(i+1)*diag_blocksize, k*diag_blocksize:(k+1)*diag_blocksize] @ X[k*diag_blocksize:(k+1)*diag_blocksize]
