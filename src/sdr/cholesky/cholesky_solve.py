...
def chol_slv_tridiag(
    ...
    for i in range(1, n_blocks, 1):
        ...
        Y[i * blocksize : (i + 1) * blocksize] = la.solve_triangular(
            ...
            - L[i * blocksize : (i + 1) * blocksize, (i - 1) * blocksize : i * blocksize]
...
def chol_slv_tridiag_arrowhead(
    ...
    for i in range(0, n_diag_blocks):
        ...
        Y[i * diag_blocksize : (i + 1) * diag_blocksize] = la.solve_triangular(
            ...
            - L[i * diag_blocksize : (i + 1) * diag_blocksize, (i - 1) * diag_blocksize : i * diag_blocksize]
    ...
    Y[-arrow_blocksize:] = la.solve_triangular(L[-arrow_blocksize:, -arrow_blocksize:], B_temp, lower=True)

    # ----- Backward substitution -----
    # X_{ndb+1} = L_{ndb+1,ndb+1}^{-T} (Y_{ndb+1})
    X[-arrow_blocksize:] = la.solve_triangular(
        L[-arrow_blocksize:, -arrow_blocksize:], Y[-arrow_blocksize:], lower=True, trans="T"
    )
    ...
    X[-arrow_blocksize - diag_blocksize : -arrow_blocksize] = la.solve_triangular(
        ...
        - L[-arrow_blocksize:, -arrow_blocksize - diag_blocksize : -arrow_blocksize].T @ X[-arrow_blocksize:],
        lower=True,
        trans="T",
    )
    ...
    for i in range(n_diag_blocks - 2, -1, -1):
        ...
        Y_temp = (
            ...
            - L[-arrow_blocksize:, i * diag_blocksize : (i + 1) * diag_blocksize].T @ X[-arrow_blocksize:]
        )
...
def chol_slv_ndiags(
    ...
    for i in range(1, n_blocks, 1):
        ...
        for k in range(max(0, i - n_offdiags_blk), i, 1):
            B_temp = (
                B_temp
                - L[i * blocksize : (i + 1) * blocksize, k * blocksize : (k + 1) * blocksize]
                @ Y[k * blocksize : (k + 1) * blocksize]
            )

        Y[i * blocksize : (i + 1) * blocksize] = la.solve_triangular(
            L[i * blocksize : (i + 1) * blocksize, i * blocksize : (i + 1) * blocksize], B_temp, lower=True
        )
    ...
    for i in range(n_blocks - 2, -1, -1):
        ...
        for k in range(i + 1, min(i + 1 + n_offdiags_blk, n_blocks)):
            Y_temp = (
                Y_temp
                - L[k * blocksize : (k + 1) * blocksize, i * blocksize : (i + 1) * blocksize].T
                @ X[k * blocksize : (k + 1) * blocksize]
            )

        X[i * blocksize : (i + 1) * blocksize] = la.solve_triangular(
            L[i * blocksize : (i + 1) * blocksize, i * blocksize : (i + 1) * blocksize], Y_temp, lower=True, trans="T"
        )
...
def chol_slv_ndiags_arrowhead(
    ...
    for i in range(1, n_diag_blocks, 1):
        ...
        for k in range(max(0, i - n_offdiags_blk), i, 1):
            B_temp = (
                B_temp
                - L[i * diag_blocksize : (i + 1) * diag_blocksize, k * diag_blocksize : (k + 1) * diag_blocksize]
    ...
    for i in range(n_diag_blocks - 2, -1, -1):
        ...
        for k in range(i + 1, min(i + 1 + n_offdiags_blk, n_diag_blocks)):
            Y_temp = (
                Y_temp
                - L[k * diag_blocksize : (k + 1) * diag_blocksize, i * diag_blocksize : (i + 1) * diag_blocksize].T
                @ X[k * diag_blocksize : (k + 1) * diag_blocksize]
            )

        # Y_tmp = Y_tmp - L_{ndb+1,i}^T X_{ndb+1}
        Y_temp = Y_temp - L[-arrow_blocksize:, i * diag_blocksize : (i + 1) * diag_blocksize].T @ X[-arrow_blocksize:]
