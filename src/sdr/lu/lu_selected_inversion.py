...
def sinv_ndiags_greg(
    ...
    for k in range(n):
        ...
        sinv_cdag.add(
            [Vertex("A", A, (k, k), output=False)], Vertex("A", A, (k, k), output=True)
        )
        A[k, k] = (
            1 / A[k, k]
        )  # <- this is NOT the part of LU! This is already a first step of INVERSION
        ...
        sinv_cdag.add(
            [Vertex("A", A, (k, k), output=False)],
            Vertex("A_inv_str", A_inv_str, (k, k), output=True),
        )
