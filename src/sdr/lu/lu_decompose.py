...
from tracing import lu_dcmp_explicit, MMM_explicit, invert_LU_explicit, subs_explicit

def lu_dcmp_tridiag_explicit(
    in_A: np.ndarray,
    blocksize: int,
) -> np.ndarray:
    """ Perform the LU factorization of a block tridiagonal matrix. The 
    ...
    for i in range(nblocks-1):
        # L_{i, i}, U_{i, i} = lu_dcmp(A_{i, i})
        # LU decomposition
        L[i*blocksize:(i+1)*blocksize, i*blocksize:(i+1)*blocksize], U[i*blocksize:(i+1)*blocksize, i*blocksize:(i+1)*blocksize] = \
            lu_dcmp_explicit(A[i*blocksize:(i+1)*blocksize, i*blocksize:(i+1)*blocksize])

        # LU inversion
        LU = L[i*blocksize:(i+1)*blocksize, i*blocksize:(i+1)*blocksize] + \
                U[i*blocksize:(i+1)*blocksize, i*blocksize:(i+1)*blocksize] - np.eye(blocksize)
        # L[i*blocksize:(i+1)*blocksize, i*blocksize:(i+1)*blocksize], U[i*blocksize:(i+1)*blocksize, i*blocksize:(i+1)*blocksize] = \
        L_inv, U_inv = invert_LU_explicit(LU)
        
        # L_{i+1, i} = A_{i+1, i} @ U{i, i+1}^{-1}
        L[(i+1)*blocksize:(i+2)*blocksize, i*blocksize:(i+1)*blocksize] = \
            MMM_explicit(A[(i+1)*blocksize:(i+2)*blocksize, i*blocksize:(i+1)*blocksize], 
                U_inv)
        ...
        # L_{i, i+1} = L{i+1, i}^{-1} @ A_{i, i+1} 
        U[i*blocksize:(i+1)*blocksize, (i+1)*blocksize:(i+2)*blocksize] =  \
            MMM_explicit(L_inv, #L[i*blocksize:(i+1)*blocksize, i*blocksize:(i+1)*blocksize],
                 A[i*blocksize:(i+1)*blocksize, (i+1)*blocksize:(i+2)*blocksize])
        # U[i*blocksize:(i+1)*blocksize, (i+1)*blocksize:(i+2)*blocksize] =  la.solve_triangular(L[i*blocksize:(i+1)*blocksize, i*blocksize:(i+1)*blocksize], np.eye(blocksize), lower=True) @ A[i*blocksize:(i+1)*blocksize, (i+1)*blocksize:(i+2)*blocksize]

        # A_{i+1, i+1} = A_{i+1, i+1} - L_{i+1, i} @ U_{i, i+1}
        A[(i+1)*blocksize:(i+2)*blocksize, (i+1)*blocksize:(i+2)*blocksize] = \
            subs_explicit(A[(i+1)*blocksize:(i+2)*blocksize, (i+1)*blocksize:(i+2)*blocksize],
            MMM_explicit(L[(i+1)*blocksize:(i+2)*blocksize, i*blocksize:(i+1)*blocksize], 
                U[i*blocksize:(i+1)*blocksize, (i+1)*blocksize:(i+2)*blocksize]))

    # L_{nblocks, nblocks}, U_{nblocks, nblocks} = lu_dcmp(A_{nblocks, nblocks})
    L[-blocksize:, -blocksize:], U[-blocksize:, -blocksize:] = lu_dcmp_explicit(A[-blocksize:, -blocksize:])
    # L[-blocksize:, -blocksize:], U[-blocksize:, -blocksize:] = la.lu(A[-blocksize:, -blocksize:], permute_l=True)
