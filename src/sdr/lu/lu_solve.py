...
def lu_slv_tridiag_arrowhead(
    ...
    X[-arrow_blocksize - diag_blocksize : -arrow_blocksize] = la.solve_triangular(
        ...
        - U[-arrow_blocksize - diag_blocksize : -arrow_blocksize, -arrow_blocksize:] @ X[-arrow_blocksize:],
        lower=False,
    )
    ...
    for i in range(n_diag_blocks - 2, -1, -1):
        ...
        Y_temp = (
            ...
            - U[i * diag_blocksize : (i + 1) * diag_blocksize, -arrow_blocksize:] @ X[-arrow_blocksize:]
        )
...
def lu_slv_ndiags_arrowhead(
    ...
    for i in range(n_diag_blocks - 2, -1, -1):
        ...
        for k in range(i + 1, min(i + 1 + n_offdiags, n_diag_blocks)):
            Y_temp = (
                Y_temp
                - U[i * diag_blocksize : (i + 1) * diag_blocksize, k * diag_blocksize : (k + 1) * diag_blocksize]
                @ X[k * diag_blocksize : (k + 1) * diag_blocksize]
            )

        # Y_tmp = Y_tmp - U_{i,ndb+1} X_{ndb+1}
        Y_temp = Y_temp - U[i * diag_blocksize : (i + 1) * diag_blocksize, -arrow_blocksize:] @ X[-arrow_blocksize:]
