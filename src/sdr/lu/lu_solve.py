...
def lu_slv_ndiags(
    ...
    for i in range(n_blocks-2, -1, -1):
        ...
        for k in range(i+1, min(i+1+n_offdiags, n_blocks)):
            Y_temp = Y_temp - U[i*blocksize:(i+1)*blocksize, k*blocksize:(k+1)*blocksize] @ X[k*blocksize:(k+1)*blocksize]
...
def lu_slv_ndiags_arrowhead(
    ...
    for i in range(n_diag_blocks-2, -1, -1):
        ...
        for k in range(i+1, min(i+1+n_offdiags, n_diag_blocks)):
            Y_temp = Y_temp - U[i*diag_blocksize:(i+1)*diag_blocksize, k*diag_blocksize:(k+1)*diag_blocksize] @ X[k*diag_blocksize:(k+1)*diag_blocksize]
