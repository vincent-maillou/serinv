...
def getcost_bta_seq_factorization(n_blocks, diag_blocksize, arrowhead_blocksize):
    ...
    matmult_cost = (
        3 * (n_blocks - 1) * diag_blocksize**3
        + 4 * (n_blocks - 1) * diag_blocksize**2 * arrowhead_blocksize
        + (n_blocks - 1) * diag_blocksize * arrowhead_blocksize**2
    )
    addsub_cost = (
        (n_blocks - 1) * diag_blocksize**2
        + (n_blocks - 1) * diag_blocksize * arrowhead_blocksize
        + (n_blocks - 1) * arrowhead_blocksize**2
    )
...
def getcost_bta_seq_sinv(n_blocks, diag_blocksize, arrowhead_blocksize):
    triangular_solve_cost = 2 * n_blocks * diag_blocksize**2
    matmult_cost = (
        (6 * (n_blocks - 1) + 3) * diag_blocksize**3
        + 7 * (n_blocks - 1) * diag_blocksize**2 * arrowhead_blocksize
        + 2 * n_blocks * diag_blocksize * arrowhead_blocksize**2
    )
    addsub_cost = (4 * (n_blocks - 1) + 1) * diag_blocksize**2 + 2 * (
        n_blocks - 1
...
def getcost_bta_middle_dist_factorization(
    ...
    matmult_cost = (
        (8 * (n_blocks_partition - 2) + 4) * diag_blocksize**3
        + (6 * (n_blocks_partition - 2) + 4) * diag_blocksize**2
        + arrowhead_blocksize
        + (n_blocks_partition - 2) * diag_blocksize
        + arrowhead_blocksize**2
    )
    addsub_cost = (
        2 * (n_blocks_partition - 2) * diag_blocksize**2
        + 4 * (n_blocks_partition - 2) * diag_blocksize * arrowhead_blocksize
        + (n_blocks_partition - 2) * arrowhead_blocksize**2
    )

    bta_middle_dist_factorization = (
        lu_cost + triangular_solve + matmult_cost + addsub_cost
    )
...
def getcost_bta_middle_dist_sinv(
    ...
    matmult_cost = (
        15 * (n_blocks_partition - 1) * diag_blocksize**3
        + 11 * (n_blocks_partition - 1) * diag_blocksize**2 * arrowhead_blocksize
        + 2 * (n_blocks_partition - 1) * diag_blocksize * arrowhead_blocksize**2
    )
    addsub_cost = (
        11 * (n_blocks_partition - 1) * diag_blocksize**2
        + 4 * (n_blocks_partition - 1) * diag_blocksize * arrowhead_blocksize
    )
...
if __name__ == "__main__":
    ...
    for n_blocks in range(1, max_n_blocks):
        for n_partitions in range(1, max_partitions):
            # Compute the cost of the BTA sequential algorithm
            cost_bta_seq_factorization = getcost_bta_seq_factorization(
                n_blocks, diag_blocksize, arrowhead_blocksize
            )
            cost_bta_seq_sinv = getcost_bta_seq_sinv(
                n_blocks, diag_blocksize, arrowhead_blocksize
            )
            ...
            cost_bta_dist_middle_process_factorization = (
                getcost_bta_middle_dist_factorization(
                    n_blocks_partition, diag_blocksize, arrowhead_blocksize
                )
            )
            cost_bta_dist_reduced_system_solve = getcost_bta_seq_factorization(
                reduced_system_size, diag_blocksize, arrowhead_blocksize
            ) + getcost_bta_seq_sinv(
                reduced_system_size, diag_blocksize, arrowhead_blocksize
            )
            cost_bta_dist_middle_process_sinv = getcost_bta_middle_dist_sinv(
                n_blocks_partition, diag_blocksize, arrowhead_blocksize
            )
            cost_bta_dist = sp.simplify(
                cost_bta_dist_middle_process_factorization
                + cost_bta_dist_reduced_system_solve
                + cost_bta_dist_middle_process_sinv
            )
    ...
    print(
        f"diagonal_blocksize {diag_blocksize}, arrowhead_blocksize {arrowhead_blocksize}"
    )
    ...
    for i, break_even_point in enumerate(break_even_points):
        non_zero_indices = np.where(optimal_n_blocks[i] != 0)[
            0
        ]  # Find indices of non-zero values
        plt.plot(
            non_zero_indices,
            optimal_n_blocks[i, non_zero_indices],
            label=f"break-even ratio >= {break_even_point}",
        )

    plt.title(
        "Number of blocks from which each processes of BTA-dist are doing less than the break-even ration work then BTA-seq"
    )
    ...
    ax.set_title(
        f"Ratio of the computational cost of BTA-seq over BTA-dist \n diag_blocksize = {diag_blocksize} \n arrowhead_blocksize = {arrowhead_blocksize}"
    )
