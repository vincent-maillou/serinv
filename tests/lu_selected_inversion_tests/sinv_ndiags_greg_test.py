...
def cut_to_banded(A: np.ndarray, ndiags: int):
...
def create_banded_matrix(matrix_size: int, ndiags: int):
    A = np.zeros((matrix_size, matrix_size), dtype=np.float32)
    assert matrix_size >= 2 * ndiags + 1
    tmp = np.random.randint(
        1, 10, size=(matrix_size, 2 * ndiags + 1)
    )  # + 1j * np.random.rand(matrix_size, 2*ndiags+1)
...
def test_sinv_ndiags_greg(matrix_size: int, ndiags: int):
    matrix_size = 6
    ndiags = 1
    ...
    assert (
        np.linalg.norm(reference_inverse @ A - np.eye(matrix_size)) / np.linalg.norm(A)
        < 1e-7
    )
